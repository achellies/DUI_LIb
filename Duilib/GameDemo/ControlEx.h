...
public:
    ...
    DeskListUI(CPaintManagerUI* pManager) : m_uButtonState(0), m_dwDelayDeltaY(0), m_dwDelayNum(0), m_dwDelayLeft(0)
    {
        for(int i = 0; i < 500; ++i) 
        {
            CDialogBuilder builder;
            CContainerUI* pDesk = static_cast<CContainerUI*>(builder.Create(_T("desk.xml"), (UINT)0, NULL, pManager));
            if( pDesk ) {
                this->Add(pDesk);
...
public:
    CControlUI* CreateControl(LPCTSTR pstrClass, CPaintManagerUI* pManager) 
    {
        ...
        else if( _tcscmp(pstrClass, _T("DeskList")) == 0 ) return new DeskListUI(pManager);
        return NULL;
    }
};
