...
public:
    ...
    LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
		...
		tString tstrSkin = CPaintManagerUI::GetInstancePath() + _T("skins\\GameSkin\\");
		m_pm.SetResourcePath(tstrSkin.c_str());

		tstrSkin = m_pm.GetResourcePath() + _T("login.xml");
...
public:
    ...
    void Notify(TNotifyUI& msg)
    {
        if( msg.sType == _T("windowinit") ) OnPrepare();
            ...
            GameListUI* pGameList = static_cast<GameListUI*>(m_pm.FindControl(_T("gamelist")));
            if((pGameList != NULL) &&  pGameList->GetItemIndex(msg.pSender) != -1 )
            {
    ...
    LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
		...
		tstrSkin = m_pm.GetResourcePath() + _T("hall.xml");
		CControlUI* pRoot = builder.Create(tstrSkin.c_str(), (UINT)0, &cb, &m_pm);
        ASSERT(pRoot && _T("Failed to parse XML"));
        m_pm.AttachDialog(pRoot);
        m_pm.AddNotifier(this);
