...
bool CListUI::SelectItem(int iIndex, bool bSendNofitied)
{
    ...
    if(bSendNofitied && ( m_pManager != NULL )) {
        m_pManager->SendNotify(this, kItemSelect);
    }
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    if( _tcsicmp(pstrName, _T("headerbkimage")) == 0 ) GetHeader()->SetBkImage(pstrValue);
...
void CListElementUI::DrawItemBk(void* ctx, const RECT& rcItem)
{
    ...
    if ( iBackColor != 0 ) {
        CRenderEngine::DrawColor(ctx, m_rcItem, iBackColor);
    }
    ...
    if( m_sBkImage.IsEmpty() ) {
        if( !pInfo->sImage.IsEmpty() ) {
            if( !DrawImage(ctx, (LPCTSTR)pInfo->sImage) ) pInfo->sImage.Empty();
            else return;
        }
    }
    ...
    if ( pInfo->dwLineColor != 0 ) {
        ...
        CRenderEngine::DrawLine(ctx, rcLine, 1, pInfo->dwLineColor);
    }
}
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN )
    {
		if( IsEnabled() ){
			...
			if ((pControl != NULL) && (pControl->GetInterface(kButtonUIInterfaceName) != NULL))
