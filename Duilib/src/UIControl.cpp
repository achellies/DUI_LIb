    ...
    if( m_dwBackColor != 0 ) {
        if( m_dwBackColor2 != 0 ) {
            if( m_dwBackColor3 != 0 ) {
                ...
                CRenderEngine::DrawGradient(ctx, rc, m_dwBackColor, m_dwBackColor2, true, 8);
                rc.top = rc.bottom;
                rc.bottom = m_rcItem.bottom;
                CRenderEngine::DrawGradient(ctx, rc, m_dwBackColor2, m_dwBackColor3, true, 8);
            }
            else 
                CRenderEngine::DrawGradient(ctx, m_rcItem, m_dwBackColor, m_dwBackColor2, true, 16);
        }
        else if( m_dwBackColor >= 0xFF000000 ) CRenderEngine::DrawColor(ctx, m_rcPaint, m_dwBackColor);
        else CRenderEngine::DrawColor(ctx, m_rcItem, m_dwBackColor);
    }
}
	...
	if( m_nBorderSize > 0 )
	{
        if( m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0 )
		{
			if (IsFocused() && m_dwFocusBorderColor != 0)
				CRenderEngine::DrawRoundRect(ctx, m_rcItem, m_nBorderSize, m_dwFocusBorderColor, m_cxyBorderRound.cx, m_cxyBorderRound.cy);
			else
				CRenderEngine::DrawRoundRect(ctx, m_rcItem, m_nBorderSize, m_dwBorderColor, m_cxyBorderRound.cx, m_cxyBorderRound.cy);
		}
		else
		{
			if (IsFocused() && m_dwFocusBorderColor != 0)
				CRenderEngine::DrawRect(ctx, m_rcItem, m_nBorderSize, m_dwFocusBorderColor);
			else if (m_dwBorderColor != 0)
				CRenderEngine::DrawRect(ctx, m_rcItem, m_nBorderSize, m_dwBorderColor);
		}
	}
}
