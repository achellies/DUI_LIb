...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, CPaintManagerUI* pManager, STRINGorID type, DWORD mask)
{
	if (pManager == NULL)
		return NULL;
    ...
    if( HIWORD(bitmap.m_lpstr) != NULL ) {
        ...
        HRSRC hResource = ::FindResource(pManager->GetResourceDll(), bitmap.m_lpstr, type.m_lpstr);
...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, CControlUI* pControl, CPaintManagerUI* pManager, STRINGorID type, DWORD mask)
{
...
bool CRenderEngine::CaculateImageRect(LPCTSTR pStrImage, CControlUI* pControl, CPaintManagerUI* pManager, RECT& rcImage)
{
   ...
   // 1、aaa.jpg
    ...
    if( sImageResType.IsEmpty() ) {
		STRINGorID type = (UINT) 0;
        data = LoadImage(fileName, pControl, pManager, type, dwMask);
    }
    else {
		STRINGorID type(sImageResType);
        data = LoadImage(fileName, pControl, pManager, type, dwMask);
    }
