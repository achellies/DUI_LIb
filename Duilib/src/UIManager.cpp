...
DWORD CPaintManagerUI::m_dwDefalutDisabledColor = 0xFFA7A6AA;
DWORD CPaintManagerUI::m_dwDefalutFontColor = 0xFF000000;
DWORD CPaintManagerUI::m_dwDefalutLinkFontColor = 0xFF0000FF;
DWORD CPaintManagerUI::m_dwDefalutLinkHoverFontColor = 0xFFD3215F;
DWORD CPaintManagerUI::m_dwDefalutSelectedBkColor = 0xFFBAE4FF;
...
bool CPaintManagerUI::IsOpacity() const
{
	return m_bOpacity;
}

BYTE CPaintManagerUI::GetTransparency() const
{
	return m_transparency;
}

void CPaintManagerUI::SetOpacity(bool bOpacity)
{
	m_bOpacity = bOpacity;
}

void CPaintManagerUI::SetTransparency(BYTE transparency)
{
...
CStdString CPaintManagerUI::GetInstancePath()
{
	...
	if (_tcslen(tszModule) > 0)
	{
		tstrModule = tszModule;
		tString::size_type nPos = tstrModule.find_last_of(_T('\\'));
		if (nPos != tString::npos)
		{
			tstrModule = tstrModule.substr(0, nPos + 1);
		}
	}
	return tstrModule.c_str();
}
...
{
	if( m_pParentResourcePM ) return m_pParentResourcePM->GetResourcePath();

	if (!m_pStrResourcePath.IsEmpty() && (m_pStrResourcePath.GetAt(m_pStrResourcePath.GetLength() - 1) != _T('\\')))
...
{
	if( m_pParentResourcePM ) return m_pParentResourcePM->GetResourceZip();
			...
			if( m_bUpdateNeeded ) {
				...
				if( !::IsRectEmpty(&rcClient) ) {
					...
					if( m_bFirstLayout ) {
						...
						SendNotify(m_pRoot, kWindowInit);
					}
				}
			}
...
bool CPaintManagerUI::AttachDialog(CControlUI* pControl)
{
	...
	if( m_pRoot != NULL ) {
		...
		m_pRoot->SetManager(this, NULL, false);
	...
	// Add WS_EX_LAYERED from this window styles 
	// Windows 2000/XP: Creates a layered window. Note that this cannot be used for child windows. Also, this cannot be used if the window has a class style of either CS_OWNDC or CS_CLASSDC. 
	if( ( WS_OVERLAPPEDWINDOW & dwStyle) && !(dwStyle & WS_CHILDWINDOW) /*&& !(GetClassStyle() & (CS_CLASSDC | CS_OWNDC))*/)
	{
		DWORD dwNewStyle = dwStyle;
		if ((!m_bOpacity && (m_transparency >= 0 && m_transparency < 256))) dwNewStyle |= WS_EX_LAYERED;
		else dwNewStyle &= ~WS_EX_LAYERED;
		if(dwStyle != dwNewStyle)
		{
			::SetWindowLong(m_hWndPaint, GWL_EXSTYLE, dwNewStyle);
			if (dwNewStyle && WS_EX_LAYERED)
				SetLayeredWindowAttributes(m_hWndPaint, 0, (255 * (100 - m_transparency)) / 100, LWA_ALPHA);
		}
	}
