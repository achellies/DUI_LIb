...
void CContainerUI::SetFloatPos(int iIndex)
{
	...
	if( pControl->IsRelativePos() )
	{
		...
		if(tRelativePos.szParent.cx != 0)
		{
			...
			rcCtrl.right    = rcCtrl.left + pControl->GetWidth() + (nIncrementX*tRelativePos.nZoomXPercent/100);
			rcCtrl.bottom   = rcCtrl.top + pControl->GetHeight() + (nIncrementY*tRelativePos.nZoomYPercent/100);
		}
...
{
    if( _tcsicmp(pstrName, _T("columns")) == 0 ) SetColumns(_ttoi(pstrValue));
    else CContainerUI::SetAttribute(pstrName, pstrValue);
}
