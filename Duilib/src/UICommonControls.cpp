...
SIZE CLabelUI::EstimateSize(SIZE szAvailable)
{
    ...
    if (m_cxyFixed.cy == 0)
	{
		...
		{
			// 显示单独一行
			//return CSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo().tm.tmHeight + 4);
			return CSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + m_rcTextPadding.bottom + m_rcTextPadding.top);
		}
	}
...
{
    if( _tcsicmp(pstrName, _T("fgimage")) == 0 ) SetFgImage(pstrValue);
...
void CProgressUI::PaintStatusImage(void* ctx)
{
	...
	if( !m_sFgImage.IsEmpty() ) {
		...
		if( !DrawImage(ctx, (LPCTSTR)m_sFgImage, (LPCTSTR)m_sFgImageModify) ) m_sFgImage.Empty();
		else return;
	}
}
...
void CEditWnd::Init(IEditUI* pOwner)
{
	m_pOwner = pOwner;
	RECT rcPos = CalPos();
	UINT uStyle = WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;
	if (m_pOwner->IsPasswordMode()) uStyle |= ES_PASSWORD;
	if (m_pOwner->IsMultiLine())
	{
		uStyle |= ES_MULTILINE | ES_WANTRETURN | ES_AUTOVSCROLL | ES_NOHIDESEL;
	}
	else
	{
		uStyle |= ES_AUTOHSCROLL | ES_NOHIDESEL;
	}
#if(WINVER >= 0x0400)
...
RECT CEditWnd::CalPos()
{
	...
	RECT rcInset = m_pOwner->GetEditTextPadding();
...
LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	if (m_pOwner->GetHostedControl() != NULL)
	{
		...
		m_pOwner->GetHostedControl()->CControlUI::Invalidate();
		m_pOwner->GetManager()->SendNotify(m_pOwner->GetHostedControl(), _T("textchanged"));
	}
	return 0;
}
...
m_bPasswordMode(false), m_bMultiLine(false), m_cPasswordChar(_T('*')), m_uButtonState(0)
#if(WINVER >= 0x0400)
